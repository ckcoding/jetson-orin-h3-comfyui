...
REF = "/opt/m0/ref_user7.wav"
with open("/opt/m0/ref_user7_text.txt") as f:
    REF_TEXT = f.read().strip()
print("REF_TEXT (%d chars): %s" % (len(REF_TEXT), REF_TEXT[:40]), flush=True)
SENT = [
    "今天天气真不错，我们一起出去走走吧。",
    "人工智能正在悄悄改变我们的生活方式。",
    "床前明月光，疑是地上霜，举头望明月，低头思故乡。",
    "我用这个声音，给大家演示一下语音合成的效果。",
]
NFE = 16
SPD = 0.85
print("loading F5TTS (cuda, nfe=%d speed=%.2f)..." % (NFE, SPD), flush=True)
t0 = time.time()
f5tts = F5TTS(
    model="F5TTS_v1_Base",
    ckpt_file=MODEL,
...
_ = f5tts.infer(ref_file=REF, ref_text=REF_TEXT, gen_text="这是一段用来预热的测试句子。", nfe_step=NFE, speed=SPD, seed=7)
print("warmup done", flush=True)
total_dt = 0.0
total_dur = 0.0
for i, s in enumerate(SENT, 1):
    t0 = time.time()
    wav, sr, _ = f5tts.infer(ref_file=REF, ref_text=REF_TEXT, gen_text=s, nfe_step=NFE, speed=SPD, seed=42)
    dt = time.time() - t0
    ...
    sf.write(f"/opt/m0/f5_user7_s{i}.wav", wav, sr)
